...
bool Publisher::isPublisher() { return true; }
...
Film* Publisher::addFilm(std::string name, std::string year, std::string length, 
							  std::string price, std::string summary, std::string director, int _id)
{
	Film* newFilm = new Film(this, name, stoi(year), stoi(length), stoi(price), summary, director, _id);
