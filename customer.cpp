...
bool Customer::isPublisher() { return false; }
...
bool Customer::alreadyHasBoughtFilm(int filmId)
{
	for (int k = 0; k < purchasedFilms.size(); k++)
		if (purchasedFilms[k]->getId() == filmId)
			return true;
	return false;
}
...
Film* Customer::addFilm(std::string name, std::string year, std::string length, 
							  std::string price, std::string summary, std::string director, int _id) { throw new BadRequest(); }

int Customer::getId(void) { return id; }

int Customer::getBalance(void) { return money; }
